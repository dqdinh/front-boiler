(ns front-boiler.components.forms
  (:require [front-boiler.components.common :as common]
            [front-boiler.utils :as utils :include-macros true]
            [front-boiler.disposable :as disposable :refer [dispose]]
...
(defn wrap-managed-button-handler
  ...
  (fn [& args]
    (let [event-id (utils/uuid)]
      ...
      (binding [front-boiler.async/*uuid* event-id]
        (apply handler args)))))
